...
const RentalCommentCard: React.FC<RentalCommentCardProps> = ({ comment, onCommentUpdated, onCommentDeleted }) => {
    ...
    const handleDelete = async () => {
        ...
        setIsMenuOpen(false);
        try {
            const { count, error } = await supabase.from('rental_post_comments').delete({ count: 'exact' }).eq('id', comment.id);
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isOwner && !isEditing && (
                            <div className="relative" ref={menuRef}>
                                <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-1 -mr-1 text-gray-500 dark:text-zinc-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-full">
                                    <MoreIcon />
                                </button>
                                {isMenuOpen && (
